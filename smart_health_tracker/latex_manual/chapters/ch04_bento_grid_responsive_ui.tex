...
\section{การออกแบบการ์ดแผนที่เส้นทางจริงด้วยระบบเวกเตอร์คัสตอมเพนเตอร์}
หนึ่งในข้อจำกัดของการแสดงผลแผนที่ดาวเทียมแบบดั้งเดิม (เช่น Google Maps หรือ Mapbox) บนแอปพลิเคชันมือถือ คือการใช้ทรัพยากรหน่วยความจำและการดาวน์โหลดไทล์ภาพ (Map Tiles) ปริมาณมหาศาล ซึ่งส่งผลให้อัตราการสิ้นเปลืองพลังงานแบตเตอรี่พุ่งสูงและเกิดอาการกระตุก (UI Lag)

ทีมวิจัยจึงได้ออกแบบ \textbf{GpsRouteMapCard} โดยใช้ระบบเรนเดอร์เวกเตอร์อิสระผ่านคลาส \texttt{CustomPainter} ของ Flutter ซึ่งประมวลผลบนการ์ดจอโดยตรง (GPU Acceleration) มีองค์ประกอบทางวิศวกรรมเลย์เอาต์ดังนี้

\begin{enumerate}
    \item \textbf{การแปลงพิกัดภูมิศาสตร์สู่พิกัดผืนผ้าใบแบบปรับตัว (Adaptive Geodesic Viewport Mapping)} ระบบจะคำนวณกรอบขอบเขตพิกัด ($\text{Bounding Box: } [\phi_{\min}, \phi_{\max}] \times [\lambda_{\min}, \lambda_{\max}]$) ของทุกจุดที่บันทึกได้ แล้วแปลงสัดส่วนให้อยู่ในระนาบพิกเซลหน้าจอตามขนาดกว้างยาวของหน้าจออย่างแม่นยำ พร้อมทั้งรองรับการซูม ($\text{Zoom Factor: } 0.5\times - 5.0\times$) และการเลื่อนมุมมอง (Pan Gesture)
    \item \textbf{การวาดเส้นทางโพลีไลน์แบบเรืองแสง 2 ชั้น (Dual-Layer Neon Glow Route)} วาดเส้นทางแกนหลักด้วยเกรเดียนต์สีนีออนเขียวสลับฟ้า (\texttt{\#00E676} $\to$ \texttt{\#00E5FF}) ซ้อนทับด้วยเส้นเรืองแสงรอบนอก (Outer Blur Glow) เพื่อสร้างมิติสายตาที่เด่นชัดบนพื้นหลังสีรัตติกาล
    \item \textbf{สัญญาณบีคอนเรดาร์กะพริบคลื่นชีพจร (Pulsating Radar Beacon)} แสดงตำแหน่งปัจจุบันของผู้ใช้ด้วยวงแหวนคลื่นเรดาร์แอนิเมชันขยายตัวแบบคาบเวลา ($1.6\text{ วินาที}$) พร้อมลูกศรระบุทิศทางเข็มทิศ (Heading Azimuth) ตามองศาที่ผู้ใช้กำลังมุ่งหน้าจริง
    \item \textbf{แผงข้อมูลโทรมาตรการเดินทางสด (Telemetry HUD Bar)} แสดงผลระยะทาง GPS ควบคู่กับระยะทางจากตัวนับก้าวเพื่อการเปรียบเทียบความแม่นยำ, ความเร็วเรียลไทม์, เพซความเร็ว (นาทีต่อกิโลเมตร), และความสูงสะสม
\end{enumerate}
...
\begin{enumerate}
    ...
    \item จงอธิบายหลักการแปลงพิกัดภูมิศาสตร์ละติจูดและลองจิจูดลงบนผืนผ้าใบ \texttt{CustomPainter} ในการ์ดแผนที่ GPS
